...
$githubBuild = ConvertTo-Lf @'
      - name: Validate fixtures and prepare isolated run
        ...
        env:
          ...
          AZAPI_OBJECT_ID: ${{ vars.AZAPI_OBJECT_ID }}
...
$adoBuild = ConvertTo-Lf @'
  - pwsh: |
    ...
    env:
      ...
      AZAPI_OBJECT_ID: $(AZAPI_OBJECT_ID)
...
'@
    ...
    $crossMode = $mode.Replace('-', '-cross-')
    $crossPath = $path.Replace("$mode.yml", "$crossMode.yml")
    $crossYaml = $yaml.Replace("Initialize-Run.ps1 -Mode $mode", "Initialize-Run.ps1 -Mode $mode -Topology cross-tenant")
    $crossYaml = $crossYaml.Replace('vars.AZAPI_', 'vars.CSUTF_AZAPI_').Replace('$(AZAPI_', '$(CSUTF_AZAPI_')
    $crossYaml = $crossYaml.Replace('azureSubscription: sc-tf36922-provider', 'azureSubscription: sc-tf36922-csutf-provider')
    $crossYaml = $crossYaml.Replace("Terraform backend $mode ", "Terraform backend $crossMode ")
    $crossYaml = $crossYaml.Replace("evidence-$mode-", "evidence-$crossMode-")
    $crossOriginal = $original.Replace('vars.AZAPI_', 'vars.CSUTF_AZAPI_').Replace('$(AZAPI_', '$(CSUTF_AZAPI_')
    Assert-Equal (Get-AuthSignature $crossYaml) (Get-AuthSignature $crossOriginal) "$crossMode exact source authentication sequence"
    $files[$crossPath] = "# Generated by scripts\Export-Fixtures.ps1; cross-tenant parameterization of pinned documentation.`n$crossYaml"
}
...
Write-Host "$(if ($Check) { 'Verified' } else { 'Exported' }) four HCL fixtures and eight CI entry points, source hashes and exact authentication sequences."
